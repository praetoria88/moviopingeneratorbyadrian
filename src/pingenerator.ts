...
function randomPin(arg: number): number {
...
function validatePinUniqueDigits(number: number){
let strNum = (''+number).split(''),
    result = strNum.filter((elem, i) => strNum.indexOf(elem) == i);
return result.length != (''+number).length;
}

//Function to validate the digit not to be incremental 
function validatePinIncremental(input: number) {
